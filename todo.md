...
- [x] Data still empty after sign-in (took forever to load) - root cause: OAuth callback never called syncFromServer() after saving the session token. AppContext only syncs on mount and foreground resume, so after sign-out+sign-in the data was never re-fetched. Fixed by calling syncFromServer() at all three OAuth success paths in callback.tsx.
