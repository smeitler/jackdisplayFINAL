...
#define LV_FONT_CUSTOM_DECLARE  LV_FONT_DECLARE(montserrat_light_120) LV_FONT_DECLARE(montserrat_light_36)
...
/* Disabled: the default theme (light mode) applies a light-grey background to every
 * screen, which makes our white time label (0xFFFFFF) invisible. We manage all
 * colours manually, so no theme is needed. */
...
/* Disabled: sets bg_opa=COVER + bg_color=white on lv_obj, overriding our transparent hit containers */
